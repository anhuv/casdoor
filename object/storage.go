...
func getStorageProvider(provider *Provider, lang string) (oss.StorageInterface, error) {
	...
	if storageProvider == nil {
		return nil, fmt.Errorf("%s", i18n.Translate(lang, "storage:The provider type: %s is not supported"), provider.Type)
	}
...
func DeleteFile(provider *Provider, objectKey string, lang string) error {
	...
	if strings.Contains(objectKey, "..") {
		return fmt.Errorf("%s", i18n.Translate(lang, "storage:The objectKey: %s is not allowed"), objectKey)
	}
