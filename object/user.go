...
func AddUser(user *User, lang string) (bool, error) {
	...
	if organization == nil {
		return false, fmt.Errorf("%s", i18n.Translate(lang, "auth:the organization: %s is not found"), user.Owner)
	}
	...
	if user.Owner != "built-in" {
		...
		if applicationCount == 0 {
			return false, fmt.Errorf("%s", i18n.Translate(lang, "general:The organization: %s should have one application at least"), organization.Owner)
		}
	}

	if organization.Name == "built-in" && !organization.HasPrivilegeConsent && user.Name != "admin" {
		return false, fmt.Errorf("%s", i18n.Translate(lang, "organization:adding a new user to the 'built-in' organization is currently disabled. Please note: all users in the 'built-in' organization are global administrators in Casdoor. Refer to the docs: https://casdoor.org/docs/basic/core-concepts#how-does-casdoor-manage-itself. If you still wish to create a user for the 'built-in' organization, go to the organization's settings page and enable the 'Has privilege consent' option."))
	}
